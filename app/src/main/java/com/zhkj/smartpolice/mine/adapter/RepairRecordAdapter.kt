...
class RepairRecordAdapter : BaseRecycleAdapter<RepairRecordBean>(arrayListOf()) {
    ...
    override fun onBindViewHolder(holder: BaseRecycleViewHolder, position: Int) {
        ...
        holder.itemView.tv_order_time.text = ("( ${getData(position).repairRecordEntity?.createTime} )")
        holder.itemView.tv_content.text = "${getData(position).repairRecordEntity?.content}"
    }
