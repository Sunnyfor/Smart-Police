...
import com.zhkj.wallet.utils.DisplayUtil;

/**
 * Desc 圆形倒计时控件，圆形及背景图片将绘制在控件中央
 * 可设置border及是否显示整体进度
 * 若设置显示整体进度，则需要在xml中同步设置circleBorderWidth
 * Author JoannChen
 * Mail yongzuo.chen@foxmail.com
 * Date 2020年10月21日 11:14:59
 */
public class CircleCountDownView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        int circleImgSrc = typedArray.getResourceId(R.styleable.CircleCountDownView_circleImgSrc, R.drawable.ic_radar);

        //从中间截取一个正方形
        circleImgBitmap = BitmapFactory.decodeResource(getResources(), circleImgSrc);
        int w = circleImgBitmap.getWidth(); // 得到图片的宽，高
        int h = circleImgBitmap.getHeight();
        int cropWidth = Math.min(Integer.MAX_VALUE, Math.min(w, h));
        Bitmap.createBitmap(circleImgBitmap, (circleImgBitmap.getWidth() - cropWidth) / 2, (circleImgBitmap.getHeight() - cropWidth) / 2, cropWidth, cropWidth);
